...
from Tokenize import peptokenize
...
def create_fields(opt):
    ...
    SRC = data.Field(tokenize=t_src.tokenizer, unk_token=None)
    TRG = data.Field(tokenize=t_trg.tokenizer, unk_token=None, init_token='<sos>', eos_token='<eos>')
...
def create_dataset(opt, SRC, TRG, PROP, tr_te):
    ...
    MIC = data.Field(use_vocab=False, sequential=False, dtype=torch.float)
    HEMO = data.Field(use_vocab=False, sequential=False, dtype=torch.float)
    PCA1 = data.Field(use_vocab=False, sequential=False, dtype=torch.float)
    PCA2 = data.Field(use_vocab=False, sequential=False, dtype=torch.float)
    PCA3 = data.Field(use_vocab=False, sequential=False, dtype=torch.float)

    data_fields = [('src', SRC), ('trg', TRG), ('MIC', MIC), ('HEMO', HEMO),('PCA1', PCA1),('PCA2', PCA2),('PCA3', PCA3)]
    ...
    train_iter = MyIterator(train, batch_size=opt.batchsize, device=opt.device,
                            repeat=False, sort_key=lambda x: (len(x.src), len(x.trg), len(x.MIC), len(x.HEMO), len(x.PCA1), len(x.PCA2), len(x.PCA3)),
