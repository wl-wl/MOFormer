...
class Transformer(nn.Module):
    def __init__(self, opt, src_vocab, trg_vocab):
        super().__init__()
        self.latent_dropout = nn.Dropout(opt.reparam_dropout_rate)
        self.use_cond2dec = opt.use_cond2dec
        self.use_cond2lat = opt.use_cond2lat
    ...
    def forward(self, src, trg, cond, src_mask, trg_mask):
        ...
        z1, mu1, log_var1 = self.encoder(src, cond, src_mask)
        std = torch.exp(0.5 * log_var1)
        z1 = self.latent_dropout(mu1 + std)

        d_output = self.decoder(trg, z, cond, src_mask, trg_mask)
        d_output1 = self.decoder(trg, z1, cond, src_mask, trg_mask)

        output = self.out(d_output)
        output1 = self.out(d_output1)
        if self.use_cond2dec == True:
            output_prop, output_pep = self.prop_fc(output[:, :5, :]), output[:, 5:, :]
            output_prop1, output_pep1 = self.prop_fc(output1[:, :5, :]), output1[:, 5:, :]
        else:
            output_prop, output_pep = torch.zeros(output.size(0), 5, 1), output
            output_prop1, output_pep1 = torch.zeros(output.size(0), 5, 1), output1
        return output_prop, output_pep, mu, log_var, z, output_prop1, output_pep1, mu1, log_var1, z1
