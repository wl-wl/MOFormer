...
class MultKAN(nn.Module):
    ...
    # include mult_ops = []
    def __init__(self, width=None, grid=3, k=3, mult_arity=2, noise_scale=1.0, scale_base_mu=0.0, scale_base_sigma=1.0, base_fun='silu', symbolic_enabled=True, affine_trainable=False, grid_eps=1.0, grid_range=[-1, 1], sp_trainable=True, sb_trainable=True, device='cpu', seed=1, save_act=True,
                 sparse_init=False, auto_save=True, first_init=True, ckpt_path='./model', state_id=0, round=0):
    ...
    def plot(self, folder="./figures", beta=3, mask=False, metric='fa', scale=0.5, tick=False, sample=False, in_vars=None, out_vars=None, title=None, varscale=1.0):
        ...
        if not self.save_act:
            print('cannot plot since data2 are not saved. Set save_act=True first.')

        # forward to obtain activations
        if self.acts == None:
            if self.cache_data == None:
                raise Exception('model hasn\'t seen any data2 yet.')
        ...
        # -- Take data2 coordinates and transform them to normalized figure coordinates
    ...
    def fit(self, dataset, opt="LBFGS", steps=100, log=1, lamb=0., lamb_l1=1., lamb_entropy=2., lamb_coef=0., lamb_coefdiff=0., update_grid=True, grid_update_num=10, loss_fn=None, lr=1., start_grid_update_step=-1, stop_grid_update_step=50, batch=-1,
            small_mag_threshold=1e-16, small_reg_factor=1., metrics=None, save_fig=True, in_vars=None, out_vars=None, beta=3, save_fig_freq=1, img_folder='./video', device='cpu', singularity_avoiding=False, y_th=1000., reg_metric='fa', display_metrics=None):
        ...
        def closure():
            ...
            #     true_labels = dataset['train_label'][train_id].to(self.device)
            #
            #     # AUC
            #     auc = roc_auc_score(true_labels.cpu(), pred.cpu())
            #
            #     # 混淆矩阵计算
            #     tn, fp, fn, tp = confusion_matrix(true_labels.cpu(), pred_labels.cpu()).ravel()
            #
            #     # Specificity (SP)
            ...
            #     mcc = matthews_corrcoef(true_labels.cpu(), pred_labels.cpu())
    ...
    def prune_edge(self, threshold=3e-2, log_history=True):
        ...
        for i in range(len(self.width) - 1):
            # self.act_fun[i].mask.data2 = ((self.acts_scale[i] > threshold).permute(1,0)).float()
    ...
    def attribute(self):
        ...
        for l in range(self.depth, 0, -1):
            ...
            print(f"l: {l}")
            print(f"Length of self.edge_actscale: {len(self.edge_actscale)}")
            print(f"Length of self.subnode_actscale: {len(self.subnode_actscale)}")
    ...
    def get_act(self, x=None):
        ...
        if x == None:
            if self.cache_data != None:
                ...
                raise Exception("missing input data2 x")
